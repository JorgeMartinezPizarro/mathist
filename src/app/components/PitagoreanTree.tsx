...
export default () => {
    ...
    const [number, setNumber] = useState(BigInt(0)||"0")
    ...
    return <div>
        ...
        <TextField
            ...
            onChange={(event => {1
                ...
                if (event.target.value.toString().length <= a  && parseInt(event.target.value) >= 0)
                    try {
        ...
        { !error && <>
            ...
            {triple.square && triple.square.length === 2 && triple.square[0].length === 2  && triple.square[1].length === 2 && <>
                <hr />
                <p>The pithagorean triple generated by the number {string(number)}, which in base 3 means the path over the tree: {string(triple.path || 0)}  took {(triple.path||"").length} steps in {duration(triple.time)}</p>
                <hr />
