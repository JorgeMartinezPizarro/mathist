...
export default function testMersenne(local: boolean): string[] {
    ...
    //const primes = eratosthenes(n, n).primes.filter(p => p > 19900)
    const mersennePrimes = primes.filter(prime => {
    try {
        return lucasLehmerTest(Number(prime))
    } catch (e) {
    ...
    ...mersennePrimes.map(prime => "<tr>" + 
        "<td style='text-align: center;'>" + ++c + "-th</td><td style='text-align: center;'>" + prime + "</td><td style='text-align: center;'>2**" + prime + " - 1</td><td style='text-align: center;'>" + (BigInt(2)**BigInt(prime)).toString().length + "</td>" +
    "</tr>"),
     "</tbody></table><hr/>",
    ]
}